...
Ce modèle est appelé un modèle \keyword{Bi-gramme}.
...
Le modèle généralisé est appelé \keyword{N-gramme}.
...
Si on veut utiliser un modèle Bi-gramme, on doit encercler chaque phrase par \keyword{\textless s\textgreater} et \keyword{\textless/s\textgreater}.
...
\text{\textit{NaN}}
\]
Les mots qui ne figurent pas dans le vocabulaire sont appelés \keyword{Out-of-vocabulary words}. 
Afin de régler ce problème, on peut ajouter un autre mot \keyword{\textless UNK\textgreater} au vocabulaire. 
...
Dans l'exemple précédent, nous avons vu le problème des mots qui n'appartiennent pas au vocabulaire (\expword{nous}) qui résultent à une division sur $0$. 
Ce dernier problème peut être résolu en réservant un mots clé pour les mots absents (comme présent précédemment).
Le problème des N-grammes absents (\expword{peut aider}), qui résultent à une probabilité de $0$, ne peut pas être résolu par cette technique. 
...
Une solution de tous ça est le \keyword{lissage}.
...
Dans le cas général, ceci est appelé \keyword{lissage de Lidstone}. 
Lorsque $\alpha = 1$, il est appelé \keyword{lissage de Laplace}. 
Si $\alpha = 0.5$, il est appelé \keyword{loi de Jeffreys-Perks}.
\begin{equation}
...
Cette représentation est appelé \keyword{One-Hot} représentation. 
...
Ce vecteur est appelé \keyword{embedding} (sera discuté en détail dans le chapitre de sémantique lexicale). 
...
Il ne faut pas oublier d'utiliser le mot clé \keyword{\textless UNK\textgreater} afin d'entraîner le modèle à prendre en considération les mots inconnus. 
...
Une solution est d'utiliser des architectures plus avancées comme \keyword{LSTM} et \keyword{GRU}. 
