...
\begin{figure}[ht]
	...
	\caption[Alphabet phonétique internationale : IPA]{Alphabet phonétique internationale\footnote{Alphabet phonétique internationale : \url{https://www.internationalphoneticassociation.org/IPAcharts/IPA_chart_orig/pdfs/IPA_DejaVu_2020_full.pdf}}}
	\label{fig:ipa}
\end{figure}
...
\begin{itemize}
	...
	Un exemple d'une flexion (dans ce cas : conjugaison) par préfixes : \expword{\RL{_dhb} /dhahaba/} (il est allé, passé), \expword{\RL{y_dhb} /yadhhabu/} (il va, présent).
	Conjugaison par préfixe et transfixe : \expword{\RL{qAl} /qāla/} (il a dit, passé), \expword{\RL{yqwl} /yaqūlu/} (il dit, présent).
...
\begin{itemize}
	...
	Exemple, \expword{happy (ADJ), unhappy (ADJ), unhapyness (N)}; \expword{\RL{jhd} /jahada/ (V), \RL{ijthd} /ijtahada/ (V)}
...
Prenons un exemple : imaginons qu'on est dans un anniversaire et une personne a créé ``\expword{Les bougie!}". 
...
\begin{itemize}
	...
	Dans la recherche d'information, si on cherche le terme ``\expword{formation}" il se peut qu'il existe des pages contenant des résultats intéressants mais avec le mot ``\expword{former}". 
...
\begin{itemize}
	...
	\item \optword{Intention} : il existe des phrases qui ne veulent pas dire ce qu'on comprenne directement ; elles veulent une autres chose
...
\begin{itemize}
	...
	Par exemple, on a remarqué que les \keywordpl[E]{embedding} (représentation des mots) attribuent des jobs comme ``docteur" aux hommes et ``infirmière" aux femmes. 
