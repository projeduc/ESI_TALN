...
L'analyse syntaxique aide à détecter si une phrase est grammaticalement correcte ou non. 
Si oui, on essaye de trouver sa structure afin d'aider d'autres tâches comme :
\begin{itemize}
...
On parle ici d'une phrase grammaticalement correcte ; ça ne veut pas dire que la phrase doit avoir un sens. 
...
Si on revient au chapitre des modèles de langage, on peut déduire que le déterminant possède une grande probabilité d'être suivi par un nom ou un adjectif suivi par un nom. 
On appelle cette structure (En RegEx. \expword{DET ADJ* NOM}) un syntagme nominal. 
Il est appelé ``nominal" et pas ``adjectival" ou ``déterminant" puisque le nom ici c'est le centre le la structure. 
L'adjectif modifie souvent un nom et un déterminant est toujours dépendant d'un nom. 
Il existe plusieurs syntagmes selon la catégorie noyau : nominal, verbal, adjectival, prépositionnel, etc.
Par exemple, \expword{[Le/DET cours/NOM ]\textsubscript{NP} [est/V intéressant/ADJ VP]\textsubscript{VP}}.
la composition de plusieurs syntagmes forment un autre syntagme jusqu'à arriver à la phrase. 
Techniquement, une phrase peut être structurée comme un arbre où la racine est la phrase, les nœuds internes sont les syntagmes et les feuilles sont les mots avec leurs catégories grammaticales.
...
La première considère la préposition ``avec" dépendante du nom ``solution", et donc on aura la dérivation suivante (Figure \ref{fig:cfg-ambigue}(1)) : 
\begin{align*}
...
\end{align*}
La deuxième considère la préposition ``avec" dépendante du verbe ``écrit", et donc on aura la dérivation suivante (Figure \ref{fig:cfg-ambigue}(2)) :
\begin{align*}
S & \sststile{}{(1)} NP\ VP \sststile{}{(2)} DT\ NN\ VP \sststile{}{(7)} DT\ NN\ AU\ VP \\
...
La première veut dire : la solution et sa solution ont été écrites par l'élève. 
...
Afin de régler l'ambigüité, on a besoin du niveau sémantique pour guider l'analyse. 
Dans notre exemple, on peut ajouter l'information que les outils d'écriture doivent être un objet concret et l'objet de l'écriture est un objet abstrait. 
\begin{figure}[ht]
	\begin{tabular}{cc}
		...
		(1) & (2) \\
	\end{tabular}
	\caption{Exemple de deux arbres syntaxique se la phrase ``L'élève a écrit une solution avec son explication" \label{fig:cfg-ambigue}}
\end{figure}

Une autre méthode pour faire face à ce problème est d'utiliser les \ac{cfg} probabilistes : \ac{pcfg}.
La grammaire est similaire de celle d'un \ac{cfg} : $G <\Sigma, N, P, S>$. 
...
Afin d'estimer cette probabilité, on utilise un corpus annoté (\keyword[T]{TreeBank}). 
...
Dans le cas d'ambigüité entre plusieurs règles, on choisit celle avec le maximum de probabilité.
...
Par exemple, dans la phrase ``\expword{Le chat mange un poisson}", le mot ``chat" est \expword{le sujet} du verbe ``mange". 
...
\begin{figure}[ht]
	...
	\caption{Exemple d'une analyse fonctionnelle de deux phrases en utilisant \url{https://corenlp.run/}\label{fig:parse-fct-exp}}
\end{figure}

La structure de dépendance est formalisée comme un graphe orienté $G=(V, A)$ où les mots représentent les sommets $V$ et les relations représentent les arcs $A$. 
...
\begin{table}[ht]
	\begin{tabular}{p{.2\textwidth}p{.35\textwidth}p{.35\textwidth}}
		...
		\hline\hline
	\end{tabular}
	\caption{Quelques relations de dépendances universelles de Stanford \cite{2014-de-marneffe-al} (\url{https://universaldependencies.org/u/dep/index.html})\label{tab:rel-dep-exp}}
\end{table}
...
Ces derniers ont des syntaxes bien définies qui peuvent être traités d'une manière déterministe. 
...
L'algorithme de \keyword{Cocke-Kasami-Younger} (CKY) utilise la programmation dynamique afin d'appliquer une analyse syntaxique ascendante. 
La seule condition pour appliquer cet algorithme est de transformer la grammaire sous la forme normale de Chomsky. 
Un petit rappel de cette forme : 
\begin{align*}
...
Après transformation de la grammaire sous FNC, on peut l'utiliser pour la reconnaissance des phrases. 
Étant donné une phrase $w = w_1 \ldots w_n$, on crée un tableau triangulaire $T$ de taille $n*n/2$. 
L'algorithme \ref{algo:cky-recon} décrit comment remplir ces deux tableaux afin d'analyser le mot $w$ suivant la grammaire $G$ en FNC. 
...
On commence par remplir le diagonal du tableau $T$ par les variables qui génèrent les mots $i$ de la phrase. 
On commence le remplissage de bas vers le haut et de gauche vers droit. 
...
Lorsqu'on arrive à la dernière cellule de la première ligne, on doit trouver l'axiome $S$ ; sinon, la phrase n'appartient pas au langage.
%\begin{algorithm}[ht]
...
Une fois la phrase acceptée et les deux tableaux créés, on utilise le tableau $B$ pour créer l'arbre syntaxique. 
On commence par choisir les deux fils de l'axiome $S$ en se basant sur $T$.
On cherche les fils de chaque nœud récursivement jusqu'à arriver à un nœud sans fils.
...
L'arbre syntaxique résultante est binaire (à cause de la forme normale de Chomsky).
Mais, dans la réalité elle doit suivre la grammaire conçue par les syntacticiens. 
Une solution est d'ajouter une étape de post-traitement pour récupérer l'arbre originale.
Concernant les productions unitaires, on peut les laisser telles qu'elles sont et modifier l'algorithme CKY afin de les accepter.
...
On va analyser la phrase ``\expword{la petite forme une petite phrase}" en utilisant CKY selon la grammaire suivante :
\begin{itemize}
...
\begin{tabular}{|p{2.3cm}|p{2.5cm}|p{2.3cm}|p{2.3cm}|p{2.5cm}|p{2.2cm}|}
	...
	\cline{2-6}
	\multicolumn{2}{l|}{}& \textbf{(V, 0, 0, 0)}; (N, 0, 0, 0) & - & (VP, 3, 1, 1) & \textbf{(VP, 3, 1, 1)} \\
	\cline{3-6}
	...
	\cline{6-6}
\end{tabular}
\caption{Exemple de l'anaylse CKY de la phrase ``la petite forme une petite phrase" \label{fig:exp-cky-trait}}
\end{figure}


\subsection{Algorithme CKY probabiliste}

Dans l'algorithme \keyword[C]{CKY}, on peut tomber sur un cas où on a plus d'un arbre syntaxique. 
Pour guider le choix des règles, on ajoute la probabilité de chaque production ; utilise une \ac{pcfg}.
...
Concernant l'algorithme \keyword[C]{CKY}, on ajoute la probabilité d'occurrence d'une règle dans chaque cellule. 
Chaque variable $A$ aura au maximum une chance pour produire deux variables.
Dans ce cas, on va créer un tableau triangulaire de trois dimensions $T[n, n, |N|]$ où on stocke la probabilité de chaque variable dans cette cellule en plus de la position $k$ utilisée pour chercher les fils et les deux variables des fils gauche et droit.
On suppose que toutes les probabilités des cellules sont initialisées par $0$.
L'algorithme \ref{algo:cky-prob-recon} représente la version probabiliste de l'algorithme CKY.
\begin{algorithm}[ht]
	\Donnees{une grammaire $G <\Sigma, N, P, S>$ en FNC; une phrase $w = w_1 \ldots w_n$}
	\Res{$T[n, n, |N|]$}
...
Dans cette arbre, tous les nœuds sont des mots et chaque arc est une relation.
On va présenter deux approches pour analyser les dépendances : par transitions et par graphes.
...
Si on arrive à la fin du mot avec une pile non vide (on considère le sommet $ROOT$ comme vide), on peut conclure que ce mot n'appartient pas au langage. 
Une transition d'un état vers un autre peut être une actions sur :
\begin{itemize}
...
Afin de décrire l'algorithme d'analyse, on va utiliser deux fonctions : ``$Oracle$" qui choisit une transition ``$t$", et ``$Appliquer$" qui exécute ``$t$" sur la configuration. 
...
Lorsqu'on est dans la configuration actuelle $ C = (\sigma, \beta, A) $, on utilise une fonction $\Psi$ qui calcule un score en utilisant sur des caractéristiques basées sur cette configuration.
...
\end{equation}
Pour entraîner le système oracle, on doit annoter le texte en le transformant vers une séquence de transitions. 
Le texte original est annoté avec les relations de dépendance. 
Pour transformer la sortie à une séquence de transitions, on utilise le système d'analyse en tel sorte à générer toutes les dépendances possibles. 
En utilisant des caractéristiques sur la configuration courante, on essaye d'estimer la transition suivante par la fonction $\Psi$.  
Cette fonction peut être MaxEnt (le plus utilisé), SVM ou les réseaux de neurones. 
...
La figure \ref{fig:arc-standard-exp} représente un exemple d'analyse de la phrase ``\expword{}" en utilisant Arc-standard.
...
\begin{figure}[ht]
	...
	\caption{Exemple de dérivations non étiquetées de la phrase ``\expword{they like bagels with lox}" en utilisant Arc-standard \cite{2018-eisenstein}\label{fig:arc-standard-exp}}
\end{figure}

Dans Arc-Eager, on essaye de détecter les relations de dépendance le plus tôt possible. 
Pour ce faire, les relations détectées doivent être entre le sommet de la pile et le mot courant. 
Dans ce cas, on n'avance pas la tête de lecture automatiquement lorsqu'on ajoute une nouvelle relation. 
Dans ce cas, la seule méthode pour le faire est d'empiler le mot danns la pile en utilisant la transition ``SHIFT". 
...
\begin{figure}[ht]
	...
	\caption{Exemple de dérivations non étiquetées de la phrase ``\expword{they like bagels with lox}" en utilisant Arc-eager \cite{2018-eisenstein}\label{fig:arc-eager-exp}}
\end{figure}

Les relations présentées ici sont des relations non étiquetées ; juste la relation tête-dépendant sans type.
Afin d'ajouter le type de relation, on doit enrichir les transitions ARC-LEFT et ARC-RIGHT avec les types possibles. 
Dans Arc-standard, à la place de 3 transitions, on aura $1+2R$ transitions où $R$ est le nombre des types de dépendances.

\subsection{Par graphe}

L'élimination des relations est une autre approche pour trouver l'arbre de dépendance d'un mot donné $w$. 
Dans un premier temps, on considère toutes les dépendances possibles entre les mots. 
Ensuite, on commence à éliminer les relations non probables jusqu'à arriver au résultat final. 
D'une façon plus formelle, on commence l'analyse par un graphe complet $G = (V, E)$ où $V$ est l'ensemble de nœuds (mots) et $E$ l'ensemble d'arcs (relations de dépendance). 
Ensuite, on cherche l'arbre $T = (V, F)$ parmi les arbres possibles $\mathcal{T}(G)$ qui est un sous-graphe de $G$ maximisant un certain score comme indiqué dans l'équation \ref{eq:arbre-rela-max}.
\begin{equation}
...
Afin de estimer le score d'un arc $e$, on peut utiliser un certain nombre de caractéristiques $f$ comme : le mot de l'entête, sa catégorie grammaticale, son lemme, ses préfixes et suffixes, la direction de l'arc, la distance entre l'entête et le dépendant, etc.
...
En utilisant des scores initiaux, on génère un arbre et en le compare avec l'arbre destinataire. 
Si les arbres ne sont pas identique, on calcule l'erreur et on mis à jours les paramètres.
...
Afin de créer un arbre à partir d'un graphe, on peut utiliser l'algorithme de Chu-Liu/Edmonds (voir l'algorithme \ref{algo:chu-liu-edmonds}).
Pour analyser un mot $w=w_1 \ldots w_n$, on commence par construire un graphe complet $G = (V, E)$ où chaque nœud $v_i$ représente un mot $w_i$ et chaque arc $e$ représente une relation possible entre deux nœuds. 
Afin de représenter l'information du mot racine, on ajoute un nœud (ROOT) avec des arcs vers tous le reste des nœuds.
Pour chaque arc $e$ du graphe $G$, on affecte un poids $G.p(e)$ qui est estimé en utilisant l'apprentissage automatique.
En prenant le nœud ``ROOT" comme racine et le graphe $G$, on essaye de trouver un arbre couvrant de poids maximal $T = (V, F)$. 
Un arbre couvrant est un sous-graphe acyclique maximal où tous les nœuds sont connectés et il n'y a pas plus qu'un arc entrant vers un nœud.
...
\begin{figure}[ht]
	...
	\caption{Exemple de l'analyse de la phrase ``Book a flight" en utilisant l'algorithme de Chu-Liu-Edmonds \cite{2019-jurafsky-martin}\label{fig:cke-exp}}
\end{figure}
...
Certes, on peut avoir les éléments de la phrase ; mais sans structure, on ne peut pas savoir leurs rôles et leurs relations. 
...
On s'intéresse à deux structures principales : constituante et fonctionnelle.
...
Afin d'analyser une telle structure, on peut utiliser l'algorithme CKY. 
...
Pour évaluer une analyse automatique, on peut utiliser le nombre des phrases avec une analyse juste. 
...
L'analyse de dépendances peut être accomplie en considérant les relations comme des transitions et donc utiliser une automate à pile pour les détecter.
Une autre approche est de considérer toutes les relations possible et d'utiliser les propriétés des graphes afin de trouver un arbre syntaxique. 
Cette dernière est plus adéquate pour les relations à long terme ; lorsque la phrase est trop longue.
Dans le cas des dépendances, on peut évaluer un arbre en utilisant le nombre des relations correctes.
\end{discussion}
