...
Afin d'estimer cette probabilité, on utilise un corpus annoté (\keyword{TreeBank}). 
...
Un mot ou plus remplissent \keyword{une fonction syntaxique} (Ex. \expword{sujet, objet, etc.}).
Ex. \expword{Le chat mange un poisson} : ici ``chat" est \expword{le sujet} du verbe ``mange". 
...
Un algorithme conçu pour l'analyse syntaxique des langages naturels est \keyword{CKY}.
...
Dans l'algorithme CKY, on peut tomber sur un cas où on a plus d'un arbre syntaxique. 
...
Concernant l'algorithme CKY, on ajoute la probabilité d'occurrence d'une règle dans chaque cellule. 
...
Prenons l'exemple de la phrase ``Book a flight".
...
Il existe plusieurs théories sur la structure syntaxique. 
