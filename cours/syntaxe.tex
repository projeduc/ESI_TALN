...
Si on revient au chapitre des modèles de langage, on peut déduire que le déterminant possède une grande probabilité d'être suivi par un nom ou un adjectif suivi par un nom (En RegEx : \expword{DET ADJ* NOM}). 
On appelle cette structure un \keyword[S]{syntagme} nominal. 
...
Afin de régler l'ambigüité, on a besoin du niveau sémantique qui peut guider l'analyse. 
Dans notre exemple, on peut ajouter l'information que les outils d'écriture doivent être des entités concrètes et l'objet de l'écriture doit être un entité abstraite. 
\begin{figure}[ht]
	\begin{tabular}{cc}
...
\begin{table}[ht]
	\begin{tabular}{p{.2\textwidth}p{.35\textwidth}p{.35\textwidth}}
		...
		\hline\hline
	\end{tabular}
	\caption[Quelques relations de dépendances universelles de Stanford]{Quelques relations de dépendances universelles de Stanford\footnote{Relations de dépendances universelles : \url{https://universaldependencies.org/u/dep/index.html} [visité le 2021-09-11]} \cite{2014-de-marneffe-al}}
...
\begin{itemize}
	\item \optword{ascendante} : à partir des mots de la phrase, on essaye de trouver les catégories grammaticales. Ensuite, les syntagmes qui génèrent une combinaison des catégories et des \keywordpl[S]{syntagme}. On fait ça jusqu'à arriver à l'axiome ``S".
	Exemple, \expword{LR}.
	\item \optword{descendante} : à partir de l'axiome ``S", on cherche les règles qui génèrent la phrase. Dans cette approche, on se base sur les mots pour guider la génération.
...
Après transformation de la grammaire $G <\Sigma, N, P, S>$ sous FNC, on peut l'utiliser pour la reconnaissance des phrases. 
...
On commence par remplir le diagonal du tableau $T$ par les variables $A$ qui génèrent les mots $w_i$ de la phrase $w$ ($A \rightarrow\ w_i$). 
On continue le remplissage du bas vers le haut et du gauche vers le droit. 
...
Une fois la phrase $w$ acceptée et le tableau rempli, on utilise ce denier pour générer l'arbre syntaxique. 
...
Si on arrive à la fin du mot avec une pile non vide (on considère le sommet $ROOT$ comme vide) sans actions de vidage, on peut conclure que ce mot n'appartient pas au langage. 
...
Lorsqu'on est dans la configuration actuelle $ C = (\sigma, \beta, A) $, on utilise une fonction $\Psi$ qui calcule un score en utilisant des caractéristiques basées sur cette configuration.
...
\end{equation}
Pour entraîner le système ``Oracle", on doit annoter le texte en le transformant à une séquence de transitions. 
...
Afin d'ajouter le type de la relation, on doit enrichir les transitions ``ARC-LEFT" et ``ARC-RIGHT" avec les types possibles. 
...
Afin d'estimer le score d'un arc $e$, on peut utiliser un certain nombre de caractéristiques $f$ comme : le mot de l'entête, sa catégorie grammaticale, son lemme, ses préfixes et suffixes, la direction de l'arc, la distance entre l'entête et le dépendant, etc.
...
En utilisant des scores initiaux, on génère un arbre et on le compare avec l'arbre destinataire. 
Si les arbres ne sont pas identiques, on calcule l'erreur et on mis à jours les paramètres.
...
En prenant le nœud ``ROOT" comme racine et le graphe $G$, on essaye de trouver un arbre $T = (V, F)$ couvrant de poids maximal. 
...
Pour évaluer une analyse automatique, on peut utiliser le nombre des phrases ayants des arbres syntaxiques justes. 
