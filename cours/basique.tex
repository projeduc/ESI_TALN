...
L'extension du domaine doit toujours être précédée par un point ; ici, on utilise \expword{/\textbackslash ./} pour indiquer qu'il s'agit du caractère point.
...
Pour toute utilisation des expressions régulières en Python, on doit faire appel au module ``re".
...
Des fois, lorsqu'on écrit en utilisant nos ordinateurs, on fait des erreurs de frappe ; on peut insérer un caractère de plus, dupliquer un caractère, etc. 
...
Étant donné deux chaînes $X$ et $Y$ avec les longueurs $n$ et $m$ respectivement, on définit un tableau $C[m, n]$ à deux dimensions contenant la longueur de la LCS.
...
Étant donné deux chaînes $X$ et $Y$ avec les longueurs $n$ et $m$ respectivement, on définit un tableau $D[m, n]$ à deux dimensions contenant la distance d'édition entre les sous-chaînes X[1..i] et Y[1..j]. 
...
Étant donné deux chaînes $X$ et $Y$ avec les longueurs $n$ et $m$ respectivement, on calcule le nombre des caractères correspondants $c$ et le nombre des transpositions $t$. 
...
\begin{itemize}
	...
	Mais, ce n'est pas toujours le cas ; on peut trouver une abréviation suivie par un nom propre (Ex. ``\expword{M. Aries}"). 
	...
	\item \optword{Classes des abréviations} : les abréviations peuvent se figurer à la fin de la phrase. 
...
Dans des tâches qui se basent sur les statistiques sur des mots (comme la recherche d'information), on doit traiter ces variations comme un seul token. 
Même dans des tâches comme la compréhension de la langue, on a besoin de trouver des formes standards des dialectes (comme ``\expword{ain't}").
...
\begin{itemize}
	\item \optword{Acronymes et les abréviations} : des fois, on trouve plusieurs variations d'une même abréviation.
	...
	Dans les tâches où on a besoin d'une compréhension plus approfondie, on doit chercher la version longue. 
	...
	Dans les tâches qui s'intéressent aux statistiques (Ex. nombre des dates dans un texte), on n'a pas besoin de garder les formes numériques. 
	...
	\item \optword{Majuscules et Minuscules} : dans la plupart des cas, on n'a pas besoin de garder les mots en majuscules (\expword{Texte \textrightarrow\ texte}). 
	...
	Par exemple, dans le français, on appelle ça désaccentuation (\expword{système \textrightarrow\ systeme}).
	Dans l'arable, on l'appelle dé-vocalisation (\expword{\RL{yadrusu} \textrightarrow\ \RL{ydrs}}). 
	...
	\item \optword{Contractions} : en général, on trouve ces formes beaucoup plus sur les réseaux sociaux. 
...
Mais, on peut avoir des cas particuliers où cela peut causer des problèmes. 
...
L'approche par règles est beaucoup plus utilisée dans ce cas, mais on peut trouver des recherches qui utilisent le niveau caractère pour appendre à générer des formes d'un mot donné, comme le projet MLConjug\footnote{MLConjug : \url{https://github.com/SekouD/mlconjug} [visité le 2021-09-08]}. 
...
\begin{itemize}
	...
	\item \optword{Modèles (\textit{template})} : dans cette solution, on stocke les conjugaisons de certains verbes comme modèles et une autre liste de tous les verbes de la langue avec leurs modèles respectifs.
	...
	Mais, on doit gérer beaucoup de règles et aussi on a besoin d'une base de données si on veut vérifier le type du verbe (il peut y avoir des verbes confondus).
...
\begin{itemize}
	...
	\item \optword{Apprentissage automatique} : On essaye d'apprendre le lemme des mots en se basant sur des critères comme leurs catégories grammaticales.
...
\begin{discussion}
	...
	Tellement elles sont simples, on peut implémenter des outils pour n'importe quelle langue en sachant les règles morphologiques. 
