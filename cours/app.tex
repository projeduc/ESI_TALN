...
L'encodeur se compose de huit couches \keyword[L]{LSTM} dont le premier est un \keyword[B]{Bi-LSTM} afin de capturer le contexte future. 
Le texte d'entrée $X = x_1 \ldots x_n$ sera encodé comme une séquence $\bar{x}_1 \ldots \bar{x}_n$. 
Le décodeur, lui aussi, se compose de 8 \keyword[L]{LSTM}s.
...
\begin{figure}[!ht]
	...
	\caption[Classification des méthodes de résumé automatique]{Classification des méthodes de résumé automatique selon \cite{98-hovy-lin,99-sparckjones}}
	\label{fig:ats-class}
\end{figure}
...
Un bloc final est utilisé inférer le score ROUGE-1 en utilisant les scores intermédiaires. 

\begin{figure}[!ht]
	\centering
	\hgraphpage[.7\textwidth]{ml2es-archi_noir.pdf}
	\caption[Architecture du système ML2ExtraSum (résumé automatique)]{Architecture du système ML2ExtraSum qui utilisent des caractéristiques avec ML \cite{2020-aries}}
	\label{fig:ats-ml2es}
\end{figure}
...
Ces vecteurs sont passés par un réseaux \keyword[B]{Bi-LSTM} afin d'extraire les \keyword[E]{embedding}s des mots par rapport au passage. 
...
S'il y a plusieurs relations comme réponse, on peut calculer la similarité entre la question et la réponse (par exemple, en utilisant les \keyword[E]{embedding}s). 
...
Les \keyword[C]{chatbot}s ont comme but d'imiter la conversation humaine.
...
Par exemple, le slot ``\expword{RESERVATION\_DATE}" du cadre ``\expword{HOTEL\_RESERVATION}" peut être rempli à partir du slot ``\expword{ARRIVAL\_DATE}" du cadre ``\expword{FLIGHT\_RESERVATION}".
...
Les \keyword[C]{chatbot}s ont comme but d'émuler une conversation humaine. 
...
Les \keyword[C]{chatbot}s à base de \ac{ri} se basent sur des corpus de conversations $C$ entre humains. 
...
\end{equation}
Les requêtes et les réponses peuvent être encodées en utilisant TF-IDF ou en utilisant les \keyword[E]{embedding}s.
Par exemple, en utilisant \keyword[B]{BERT}, on peut encoder la requête comme un vecteur $h_q$ et la réponse comme un vecteur $h_r$ (la sortie ``[CLS]") comme indiqué dans l'équation \ref{eq:chatbot-enc-bert}.
...
Un \keyword[C]{chatbot} peut être modéliser comme un problème de génération du  texte. 
...
\end{equation}
On peut aussi utiliser des modèles de langue, comme \keyword[G]{GPT} entraînés sur des conversations.
...
En entrée, on passe la texte et on attache un réseau à propagation et en sortie ``[CLS]" on entraîne le modèle à estimer le sentiment. 
...
\begin{figure}
	...
	\caption[Architecture hybride pour détecter la polarité en dialecte algérienne]{Architecture hybride proposée par \cite{18-bettiche-al} pour détecter la polarité des messages en dialecte algérienne sur les réseaux sociaux}
...
Ensuite, le corpus annoté automatiquement est utilisé afin d'entraîner un modèle de classification en utilisant des critères comme les \keyword[E]{embedding}s des mots. 
...
\begin{figure}
	...
	\caption[Architecture hybride pour l'analyse des sentiments en arabizi]{Architecture hybride proposée par \cite{18-guellil-al} pour l'analyse des sentiments des messages en arabizi sur les réseaux sociaux}
...
On doit appliquer un \optword{échantillonnage} (sampling) afin d'extraire des valeurs du signal dans une durée défie. 
...
\end{equation}
Ce score doit être maximiser ; une solution est l'utilisant des méthodes d'optimisation comme \keyword[B]{Beam Search}.
