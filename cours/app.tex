...
Ensuite, on peut concevoir des encodeurs à partir des langages naturels vers ce langage et des décodeurs de cet langage vers des langages naturels.
On peut utiliser des corpus pour entraîner un système d'apprentissage automatique à traduire d'une langue vers une autre. 
Donc, on doit se baser sur des statistiques. 
...
Ensuite, on cherche la traduction des mots $S_i$ de la langue source vers des mots $T_i$ de la langue destinataire en utilisant un dictionnaire bilingue.
Après, on applique des règles pour transformer l'arbre syntaxique de la langue source vers un arbre syntaxique de la langue destinataire (comme l'exemple de la figure \ref{fig:mt-transfert-exp}).
...
\begin{itemize}
	...
	\item \textit{morphological analyser} : segmentation du texte et attribution à chaque mot une liste des lemmes avec leurs catégories grammaticales possibles.
...
Dans la traduction automatique, on doit implémenter un système pour chaque pair de langues (source et destinataire). 
...
On peut considérer cette probabilité comme la somme des probabilités de tous les alignements $A$ possibles, comme indiqué par l'équation \ref{eq:mt-stat-align1}.
...
Des fois, on trouve des segments de la langue source qui sont toujours alignés avec d'autres en langue destinataire.
Donc, on peut calculer la probabilité d'un segment (ensemble de mots consécutifs) par rapport un autre. 
...
Donc, on peut utiliser un encodeur-décodeur basé sur un réseau de neurones récurrent ; il est appelé modèle sequence-to-sequence (seq2seq).
...
Selon le document de sortie, une méthode peut être classifiées en utilisant trois critères : la dérivation, la partialité et le format.
...
Parmi ces critères, on peut utiliser la fréquence des mots. 
...
Parmi les méthodes statistiques, on peut mentionner la méthode TCC (\textit{Topic Clustering and Classification}) proposée par \citet{13-aries-al} et implémentée dans un système appelé AllSummarizer\footnote{AllSummarizer : \url{https://github.com/kariminf/allsummarizer} [visité le 2021-09-16]}.
...
%
On utilise un ensemble des caractéristiques $f$ sur la phrase : TF (Uni-gramme, Bi-gramme), la position et la taille avant et après le pré-traitement.
...
Pour chercher le mot le plus similaire à un mot $w_i$ dans une phrase $S_2$, on utilise une similarité entre les vecteurs (cosinus par exemple) comme indiqué par l'équation \ref{eq:ats-oufaida-match}.
...
Étant donné deux termes $X$ et $Y$, on calcule l'information mutuelle comme indiqué par l'équation \ref{eq:ats-oufaida-mutinf}.
...
Un nœuds est jugé faible s'il satisfait la propriété exprimée par l'équation \ref{eq:ats-gc-nfaible}.
...
Aussi, on peut utiliser l'apprentissage pour décider si une unité (phrase) appartient au résumé ou non (problème de classement). 
...
En entrée, on a plusieurs caractéristiques comme la liste des TF des mots de la phrase ($sent\_tf\_seq$), la liste des TF des mots du document ($doc\_tf\_seq$), la taille de la phrase ($sent\_size$), etc.
...
Du même, pour chaque phrase $s$ d'un document $d$, on attribue un vecteur $\pi_{ds}$ de taille $K+J+1$. 
...
On utilise des filtres de déférentes tailles $h$ (ici, $4$ en bleu et $2$ en rouge) plusieurs fois (ici, on a utilisé $3$ différents filtres de même taille). 
...
Afin d'encoder le document, on passe les phrases par un réseaux récurrent \keyword[L]{LSTM} en commençant par la représentation de la première phrase.
...
Dans ce cas, la recherche d'une cause plutôt d'une entité nommées sera plus difficile. 
Une autre solution est d'utiliser un algorithme d'apprentissage automatique afin de noter les passages. 
Parmi les caractéristiques qu'on puisse utiliser : nombre des entités nommées du type recherché, nombre des termes de la question, la séquence la plus longue similaire à la question, le rang du document, etc.
...
Aussi, on peut utiliser des patrons comme ``\expword{\textless REP\textgreater comme \textless QES\textgreater; }" qui permettent de détecter des réponses comme ``\expword{des désordres développementaux comme l'autisme.}" à la question ``\expword{C'est quoi l'autisme ?}".
...
On utilise deux représentations, de début (S) et de fin (E), qui sont multipliés par la représentation de chaque mot et passées par une fonction softmax sur l'ensemble de tous les mots afin d'avoir la probabilité de début et celle de fin.
...
S'il y a plusieurs relations comme réponses, on peut calculer la similarité entre la question et la réponse (par exemple, en utilisant les \keywordpl[E]{embedding}). 
...
Ensuite, on le règle pour répondre aux questions à partir du modèle (pas besoin de passages). 
Dans \cite{2020-roberts-al}, on a entrainé un modèle T5 sur la tâche de remplissage des textes absents (voir la figure \ref{fig:qr-modele-t5}).
...
Lorsqu'on rempli un slot, on peut remplir les slots d'autres cadres en parallèle. 
...
Lorsqu'il y a plusieurs domaines, on doit appliquer la \keyword{détection d'intention} pour savoir quel cadre à utiliser. 
...
En fait, on peut l'exprimer en utilisant des expressions régulières.
...
Le système commence par recevoir un acte ``HELLO" et détecter qu'il s'agit d'une tâche de recherche et que le type recherché est une restaurant. 
Il lance un acte de confirmation de la requête en demandant quel est le plat préféré de l'utilisateur afin de recommander un restaurant. 
...
Afin d'extraire les informations et remplir les slots, on peut utiliser l'étiquetage des séquences avec la technique \keyword[I]{IOB}. 
...
Étant donnée une requête utilisateur $q$, on cherche la réponse $r \in C$ qui est plus similaire à la requête $q$ comme indiqué par l'équation \ref{eq:chatbot-sim}.
...
Par exemple, en utilisant \keyword[B]{BERT}, on peut encoder la requête comme un vecteur $h_q$ et la réponse comme un vecteur $h_r$ (la sortie ``[CLS]") comme indiqué par l'équation \ref{eq:chatbot-enc-bert}.
...
\end{equation}
On peut aussi utiliser des modèles de langage (comme \keyword[G]{GPT}) entraînés sur des conversations.
...
En utilisant un dictionnaire comme \keyword[W]{WordNet}, on peut enrichir une liste des indicateurs manuellement définie (synonymes, antonymes, etc.).
...
On peut utiliser un algorithme d'apprentissage automatique avec des caractéristiques comme : la présence d'un mot et sa fréquence, les catégories grammaticales des mots, les mots et les syntagmes d'opinion, la négation, etc.
...
En entrée, on passe le texte et en sortie ``[CLS]" on entraîne le modèle à estimer le sentiment. 
...
Supposons que nous avons un vocabulaire $V_p$ positif et un autre $V_n$ négatif qui sont définis manuellement. 
Pour chaque mot $w$ du vocabulaire qui n'appartient pas à ces deux vocabulaires, on calcule son orientation sémantique comme indiqué par l'équation \ref{eq:asent-bettiche-orientation}
...
En se basant sur un lexicon annoté en anglais (mots avec des scores de polarité), on traduit les mots vers l'arabe. 
...
Un des scores utilisés pour juger la lisibilité des textes en anglais, on peut mentionner \optword{Flesch-Kincaid Grade Level}. 
...
Cette métrique se base sur le nombre des mots, le nombre des phrases, le nombre des mots difficiles (plus de 5 caractères sans diacritiques), le nombre des syllabes, le nombre des mots complexes (plus de 4 syllabes) et les mots Faseeh (les mots complexes avec les lettres \RL{|', y', w', _d, .z} ou qui se termine avec \RL{wn, wA}).
\begin{align}
...
Le style de la parole peut, aussi, affecter un système de reconnaissance : est-ce que le langage doit être standard ou on peut parler avec des sons qui n'ont pas de sens, comme ``\expword{um}".
...
On doit appliquer un \optword{échantillonnage} (sampling) afin d'extraire les valeurs du signal dans une durée définie. 
...
L'approche statistique est similaire à celle de la reconnaissance de paroles, seulement on inverse les entrées et les sorties. 
...
Dans ce chapitre, on a présenté huit applications dont chaque deux représentent un type de tâches. 
