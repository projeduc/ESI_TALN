...
\end{align}
Afin de résoudre \ac{svd}, on peut utiliser des méthode de programmation dynamique comme l'algorithme de Lanczos, la décomposition QR.

La représentation d'un terme avec un vecteur des nombres réel est appelé ``\keyword[E]{embedding}". 
La plupart des \keyword[E]{embedding}s connus actuellement utilisent les réseaux de neurones. 
Malgré c'est une représentation vectorielle, j'ai choisi de présenter les \keyword[E]{embedding}s dans une section à part.
...
On va présenter deux types des \keyword[E]{embedding}s : 
\begin{itemize}
...
Le texte est séparé en tokens en utilisant ``Wordpiece" (voir \cite{2016-wu-al}).
...
Chaque token est représenté par 3 \keyword[E]{embedding}s de taille $N$ 
\begin{itemize}
...
Par exemple, \keyword[G]{GloVe} surpasse LSA dans la tâche de reconnaissance des entités nommées selon \cite{2014-pennington-al}. 
...
Une autre méthode pour la \ac{wsd}  est d'utiliser les \keyword[E]{embedding}s contextuels. 
...
Étant donné un modèle pré-entrainé, on le règle sur un corpus annoté afin de capturer les \keyword[E]{embedding}s de chaque sens. 
Pour avoir le \keyword[E]{embedding} d'un sens $v_s$, on fait la moyenne des \keyword[E]{embedding}s $c_i$ qui l'appartiennent (voir l'équation \ref{eq:wsd-embeddings-sens}).
\begin{equation}
v_s = \frac{1}{n} \sum_{i=1}^{n} c_i 
\label{eq:wsd-embeddings-sens}
\end{equation}
Lors du test, on passe la phrase par le modèle. 
Pour chaque mot $w$, on cherche les \keyword[E]{embedding}s de tous les sens et on prend le plus proche en utilisant la similarité cosinus.
