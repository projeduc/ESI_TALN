...
\begin{itemize}
	...
	\item Question/Réponse : Dans l'exemple précédent, la réponse de la question ``\expword{Qui a une bonne odeur ?}" n'est pas ``elle" mais plutôt ``la fleur".
...
Un \optword{pronoms} est un mot-outil utilisé afin de se substituer à un nom ou un syntagme nominal. 
...
Un exemple de ce genre de références :  \expword{\underline{Karim} est entré. \underline{Il} a commencé le cours.}.
...
Contrairement aux pronoms qui sont toujours des références, les \keyword[S]{syntagme}s nominaux peuvent ne pas l'être. 
...
Par exemple, \expword{\underline{Karim} a présenté et \underline{$ \phi $} expliqué le cours.}.
...
\begin{itemize}
	...
	Un anaphore peut être un pronom (Ex. \expword{\underline{Le cours} est très long. \underline{Il} prendra plus de temps.}), un syntagme nominal (Ex. \expword{J'ai rencontré \underline{Fatima}. \underline{Une personne} qui adore assister les autres.}), un zéro anaphore (Ex. \expword{\underline{Le chat} a attrapé la souris et \underline{$ \phi $} l'a mangé.}), etc.
	...
	Exemple d'un pronom (\expword{\underline{Il} est très long, \underline{ce cours}!}), Syntagme nominal (\expword{If you want \underline{a book}, take \underline{the one on the shelf}.}), etc. 
	...
	\item \optword{Antécédents partagés} : une référence vers plusieurs mots et/ou \keyword[S]{syntagme}s.
	...
	\item \optword{Syntagmes nominaux en coréférence} : deux \keyword[S]{syntagme}s nominaux où chacun est une référence vers l'autre. 
...
Les  \optword{contraintes de théorie contraignante} sont contraintes syntaxiques sur la relation mention-antécédent.
...
La \optword{récence} veut dire que les entités mentionnées plus récemment sont plus probables à être un antécédent.
...
Dans l'exemple ``\expword{\underline{John} telephoned Bill. \underline{He} lost the laptop.}", l'évènement de téléphone a été causé par le sujet ; donc le pronom ``he" revient sur le sujet. 
...
Dans cette dernière, on peut se baser sur des caractéristiques définies manuellement ou sur les \keyword[E]{embedding}s.


\subsection{Détection de mention}

Les mentions sont des entités qui peuvent être des \keyword[S]{syntagme}s nominaux ou des pronoms.
...
\begin{itemize}
	...
	Par exemple, \expword{9\%, 100\$, cent neuf}.
	
	\item On élimine les mentions qui sont une partie d'une autre (quantificateur + ``of"). 
	Exemple, \expword{a total of 177 projects, none of them, millions of people}.
...
Le système (a) est un peut plus complexe : pour chaque mot, il utilise une représentation en utilisant \keyword[G]{GloVe}, une autre en utilisant \keyword[E]{ELMo} et une troisième basée sur des caractères en utilisant les CNNs. 
...
\optword{CEAF} (Constrained entity-alignment F-Measure) utilise une mention une seule fois lors du calcul. 
...
\begin{itemize}
	...
	\item \optword{Apprentissage avec caractéristiques} : \keyword[E]{embedding}s du mot et ses voisins, les préfixes et les suffixes, l'appartenance à une liste, etc.
