...
Une de ces formes est le syntagme verbal (Ex. \expword{Si Mouloud \underline{achète un nouveau vélo}, je \underline{le ferai} aussi.}).
...
\begin{itemize}
	\item \optword{Anaphore} : une référence vers un mot ou un syntagme précédent appelé \keyword{antécédent}.
	...
	\item \optword{Cataphore} : une référence vers un mot ou un syntagme suivant appelé \keyword{postcédent}.
	...
	\item \optword{Antécédents partagés} : une référence vers plusieurs mots et/ou syntagmes.
	...
	\item \optword{Syntagmes nominaux en coréférence} : deux syntagmes nominaux où chacun est une référence vers l'autre. 
...
Cette tâche est liée à la base de connaissance utilisée ; lorsqu'on utilise Wikipédia comme base de connaissance, on appelle cette tâche \keyword{Wikification}.
