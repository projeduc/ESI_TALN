...
\begin{introduction}[LES \textcolor{white}{L}ANGUES]
	...
	Cette propriété peut être aperçue beaucoup plus dans l'anglais où des noms peuvent être transformés en verbes sans changement (Ex. ``fish"). 
	Cette tâche est utile si on veut appliquer les statistiques sur les catégories grammaticales afin d'améliorer une autre tâche comme celles de classification des textes. 
...
Après étiquetage morpho-syntaxique, on aura ``\expword{We[pronom] can[verbe] can[verbe] the[déterminant] can[nom]}".
Ceci n'est pas seulement utile pour savoir le sens du mot, mais aussi on peut voir la structure de phrase.
Essayez d'étiqueter le phrase suivante : ``\expword{Will Will will the will to Will?}".
...
\begin{itemize}
	...
	\item les catégories grammaticales peuvent être une caractéristique importante dans des tâche de classification des textes. 
...
Un langage est une séquence continue des mots ; ce sont une phénomène temporelle.
Chaque mot ou ensemble de mots possèdent des caractéristiques linguistiques, chacune a plusieurs catégories. 
...
Parmi les règles qu'on peut utiliser : \expword{Un déterminant est suivi par un nom}. 
...
\end{equation}
Dans les modèles statistiques, il existe deux types de modèles : génératif et discriminatif. 
...
Ceci revient à apprendre l'étiquette d'un mot en se basant sur ces caractéristique en plus des caractéristiques d'un nombre limité des mots avant.
Un modèle qui suit cette méthode est \keyword{Maximum-entropy Markov model}, en anglais \ac{memm}.
...
Un exemple d'une phrase en anglais étiquetée est illustré dans la figure \ref{fig:ems-exp}.
\begin{figure}[ht]
	\centering
	\hgraphpage[.8\textwidth]{exp-pos_.pdf}
	\caption{Un exemple d'étiquetage morpho-syntaxique [\url{https://corenlp.run/}]\label{fig:ems-exp}}
\end{figure}
...
La méthode utilisée pour ça est le format \keyword{IOB} (Inside, Outside, Beginning). 
Les mots qui n'appartiennent à aucune catégorie sont annotés par ``B". 
Les mots qui marquent le commencement d'une catégorie sont annotés par ``B-" suivi par le nom de catégorie. 
Les mots du milieu sont annotés par ``I-" suivi par le nom de catégorie. 
...
\begin{figure}[ht]
	...
	\caption{Un exemple de la reconnaissance d'entités nommées [\url{https://corenlp.run/}]\label{fig:ner-exp}}
\end{figure}

L'analyse syntaxique de surface (Chunking) est une autre exemple d'étiquetage des séquences.
...
Comme mentionné avant, cette tâche sert à trouver les catégories grammaticales de chaque mot dans une phrase. 
...
Est-ce qu'on détail dans les catégories ou on se contente des catégories principale ?
Les étiquettes les plus fameuses sont celles de Penn Treebank pour l'anglais : conjonction de coordination (CC), nombre cardinal (CD), déterminant (DT), etc. 
...
\end{tabular}
\caption{Les catégories grammaticales universelles selon \url{https://universaldependencies.org/u/pos/} \label{tab:pos-cat}}
\end{table}

L'étape suivante est d'annoter un corpus manuellement. 
Ce type de corpus est appelé \keyword{treebank}.
Parmi les corpus annoté, on peut citer Penn Treebank pour l'anglais (Exemple dans la figure \ref{fig:penn-exp}).
Un autre projet riche en terme de langues traitées est Universal TreeBank (\url{https://universaldependencies.org/}) \cite{2012-petrov-al}. 
...
\begin{figure}
	...
	\begin{tcolorbox}[boxrule=0.4pt,text width=.7\textwidth]
		...
		\end{alltt}
	\end{tcolorbox}
	\caption{Exemple d'un texte annoté de Penn TreeBank \cite{2003-taylor}\label{fig:penn-exp}}
\end{figure}
...
Une chaîne de Markov peut être formalisée en un ensemble d'états $Q = \{q_1, q_2, \ldots, q_n\}$, une distribution initiale des probabilités $\pi = [\pi_1, \pi_2, \ldots, \pi_n ],\, \sum_i \pi_i = 1$ et une matrice des  probabilités de transition
$
A = \begin{bmatrix}%
...
\begin{figure}[ht]
	...
	\caption{Exemple d'un chaîne de Markov pour la prédiction du temps \cite{2019-jurafsky-martin}\label{fig:cm-exp}}
\end{figure}

Ce modèle est un modèle de langage qui ne représente que les probabilités de transition. 
Dans ce cas, on peut seulement représenter la probabilité de transition d'une catégorie à ue autre. 
Si on estime la probabilité d'émission d'un mot dans chaque état (étiquette), dans ce cas on peut estimer les étiquettes en utilisant la  théorème de Bayes. 
Ce modèle est appelé : modèle de Markov caché (\ac{hmm}) ; les mots d'une séquence sont observés or les catégories sont cachées et doit être estimées. 
...
Une observation $o_t$ peut être générée dans un état $q_i$ avec une probabilités d'observation (\keyword{probabilités d'émission}) $B = b_i(o_t)$.
Un exemple d'un \ac{hmm} est illustré dans la figure \ref{fig:hmm-exp}. 
\begin{figure}[ht]
	\centering
	\hgraphpage[.6\textwidth]{exp-hmm_.pdf}
	\caption{Exemple d'une représentation HMM \cite{2019-jurafsky-martin}\label{fig:hmm-exp}}
\end{figure}

La séquence estimée $\hat{t}$ est celle qui maximise la probabilité $P(t)$ et la probabilité $P(w_i | t_i)$ selon l'équation \ref{eq:hmm-detail}.
En utilisant un modèle Bi-gramme, la probabilité $P(t)$ est calculée en utilisant les transitions de l'état initial $t_1$ jusqu'à le dernier $t_n$. 
...
Avant de parler comment une séquence $w=w_1 \ldots w_n$ est décodée selon l'équation \ref{eq:hmm-detail}, on commence par l'entrainement. 
Dans un \ac{hmm}, les états sont les tags (catégories) $t_i$ et les observations sont les mots $w_i$. 
Notons la fonction qui compte le nombre des 
La probabilité de transition $a_{i-1,i}$ de l'état $t_{i-1}$ vers $t_i$ est estimé à partir du corpus d'entraînement selon l'équation \ref{eq:pos-hmm1}.
On note \keyword{DEBUT} comme l'étiquette qui précède la première catégorie de la phrase. 
...
\pi_i = P(t_i | DEBUT) = \frac{C(DEBUT, t_i)}{C(DEBUT)} \label{eq:pos-hmm2} \\
...
La probabilité q'un pronom soit au début de la phrase est calculée comme : $\pi_{PRON} = \frac{C(PRON,\ <s>)}{C(<s>)} = \frac{3}{4} $.
...
\begin{tabular}{llllll}
	...
	\hline\hline
\end{tabular}
\caption{Un exemple des probabilités de transition et la distribution initiale \label{tab:hmm-trans-init}}
\end{table}
...
\begin{tabular}{lllllllll}
	...
	\hline\hline
\end{tabular}
\caption{Un exemple des probabilités d'émission \label{tab:hmm-emission}}
\end{table}
...
Dans chaque état $s$ de la séquence, on se base sur l'estimation précédente pour calculer les probabilité de tous les tags afin de choisir celui qui maximise la probabilité comme estimation courante. 
Il faut toujours sauvegarder l'état précédent pour revenir en fin d'algorithme. 
Ceci est appelé la recherche \keyword{Viterbi} (voir l'algorithme \ref{algo:viterbi}).

En utilisant le modèle entraîné dans l'exemple précédent, on veut trouver les tags de cette phrase : \expword{il peut aider}. 
...
\begin{table}[ht]
	...
	\begin{tabular}{llll}
		...
		\hline\hline
	\end{tabular}
	\caption{Un exemple d'exécution de l'algorithme de Viterbi \label{tab:hmm-viterbi-exp}}
\end{table}
...
Les \ac{hmm} se basent dans leurs estimations seulement sur des statistiques sur les mots. 
...
On peut utiliser des caractéristiques sur le mot actuel et les mots avant afin d'estimer l'étiquette ; comme dans les \ac{hmm}.
...
Les réseaux de neurones récurrents sont conçus pour traiter les séquence.
Nous avons vu qu'un \ac{rnn} peut être utilisé comme modèle de langage en passant le mot encodé avec One-Hot en entrée et estimant le mot suivant et en prenant en considération le contexte précédent. 
...
On peut, bien sûr, utiliser une couche cachée pour apprendre une représentation plus compacte des mots en entrée (embedding). 
...
\begin{figure}[ht]
	\centering
	\hgraphpage[.6\textwidth]{rnn-simple_.pdf}
	\caption{Un RNN simple pour l'étiquetage morpho-syntaxique \cite{2019-jurafsky-martin}\label{fig:pos-rnn1}}
\end{figure}
...
\begin{figure}[ht]
	\centering
	\hgraphpage[.6\textwidth]{rnn-stack_.pdf}
	\caption{Des RNNs empilés pour l'étiquetage morpho-syntaxique \cite{2019-jurafsky-martin}\label{fig:pos-rnn2}}
\end{figure}
...
\begin{figure}[ht]
	\centering
	\hgraphpage[.6\textwidth]{rnn-bi_.pdf}
	\caption{RNN bidirectionnel pour l'étiquetage morpho-syntaxique \cite{2019-jurafsky-martin}\label{fig:pos-rnn3}}
\end{figure}
...
Afin de preprésenter les variations morphologiques d'un mot, on doit descendre au niveau caractère.
Une idée (voir la figure \ref{fig:pos-rnn4}) est d'utiliser un modèle de langue au niveau caractère afin d'encoder un mot et combiner ce code avec le code lexicale (One-Hot ou embedding) du mot. 
La code résultat sera l'entrée du réseau récurrent (simple, empilé ou bidirectionnel).
\begin{figure}[ht]
	\centering
	\hgraphpage[0.5\textwidth]{rnn-char1_.pdf}
	\hgraphpage[0.45\textwidth]{rnn-char2_.pdf}
	\caption{RNN avec des embedding de caractères pour l'étiquetage morpho-syntaxique \cite{2019-jurafsky-martin}\label{fig:pos-rnn4}}
\end{figure}


\begin{discussion}
Chaque langue doit avoir une méthode pour identifier les entités ou les actions d'une entité. 
Les linguistes font la différence entre la fonction d'un mot dans la phrase en utilisant des catégories. 
Par exemple un nom c'est la référence à un objet, or un adjectif est un mot qui représente une propriété d'un nom. 
...
Plusieurs approches ont été utilisée pour résoudre ce problème : par règles, statistique et pa réseaux de neurones. 
...
Dans les architectures présentées dans ce chapitre, l'entrée d'un \ac{rnn} est une représentation vectorielle du mout courant. 
...
Mais la question qui se pose : comment peut-on évaluer un système d'étiquetage des séquence. 
...
Donc, les métriques d'évaluation da la classification comme la justesse (accuracy), le rappel (recall), la précision (precision), etc. 
