...
Ce modèle est appelé : modèle de Markov caché (\keyword[H]{\ac{hmm}}) ; les mots d'une séquence sont observés or les catégories sont cachées et doit être estimées. 
...
On note ``DEBUT" comme l'étiquette qui précède la première catégorie de la phrase. 
...
Les \keyword[H]{\ac{hmm}} se basent dans leurs estimations seulement sur des statistiques sur les mots. 
...
On peut utiliser des caractéristiques sur le mot actuel et les mots avant afin d'estimer l'étiquette ; comme dans les \keyword[H]{\ac{hmm}}.
...
Nous avons vu qu'un \ac{rnn} peut être utilisé comme modèle de langage en passant le mot encodé avec \keyword[O]{One-Hot} en entrée et estimant le mot suivant et en prenant en considération le contexte précédent. 
...
On peut, bien sûr, utiliser une couche cachée pour apprendre une représentation plus compacte des mots en entrée (\keyword[E]{embedding}). 
...
Une idée (voir la figure \ref{fig:pos-rnn4}) est d'utiliser un modèle de langue au niveau caractère afin d'encoder un mot et combiner ce code avec le code lexicale (\keyword[O]{One-Hot} ou \keyword[E]{embedding}) du mot. 
