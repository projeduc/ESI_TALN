...
RST modélise le discours sous formes des relations entre deux unités (des phrases ou parties des phrases) : \optword{Noyau (N)} et \optword{Satellite (S)}. 
...
Voici quelques définitions des relations RST \cite{2006-Cornish} :
\begin{itemize}
...
Les relations RST peuvent être classifiées selon la distinction thématique/présentationnelle (voir le tableau \ref{tab:rst-class-lect}). 
...
\begin{tabular}{p{.3\textwidth}p{.3\textwidth}p{.3\textwidth}}
	...
	\hline\hline
\end{tabular}
\caption{Classification des relations RST selon le point de vu lecteur \cite{2006-Cornish}}
...
\begin{tabular}{p{.3\textwidth}p{.2\textwidth}p{.2\textwidth}p{.15\textwidth}}
	...
	\hline\hline
\end{tabular}
\caption{Classification des relations RST selon les niveaux de traitement du langage \cite{2006-Cornish}}
\label{tab:rst-niveau}
\end{table}

\subsection{Penn Discourse TreeBank (PDTB)}

\ac{pdtb} est un dataset annoté avec un autre modèle de cohérence. 
...
Les relations \ac{pdtb} sont divisées en quatre familles : temporelle, comparaison, contingence et extension. 
...
Dans le cas de \ac{rst}, cette structure est un arbre indiquant les relations de cohérence entre les différentes parties. 
Contrairement à \ac{rst}, \ac{pdtb} ne génère pas un arbre.
...
L'analyse \ac{rst} consiste à construire un arbre de relations de cohérence entre les parties d'un texte. 
Cette tâche passe par deux étapes : détection des unités élémentaires de discours et classification des relations. 
La figure \ref{fig:rst-exp} représente une analyse \ac{rst} d'un texte.
...
La figure \ref{fig:edu-embedding} représente leur architecture où l'entrée est une concaténation entre deux embeddings : \keyword[G]{GloVe} et \keyword[B]{BERT}. 
On utilise un réseau Bi-LSTM afin d'avoir le contexte en avant et en arrière pour chaque mot. 
Ensuite, chaque mot est classé comme : début d'un \ac{edu} (1) ou continuation d'un \ac{edu} (0). 

\begin{figure}[!ht]
	\centering
	\hgraphpage[.6\textwidth]{EDU_seg_.pdf}
	\caption{Segmentation en EDUs proposée par \cite{2018-wang-al} ; figure prise de \cite{2019-jurafsky-martin}}
...
La méthode la plus utilisée pour trouver la structure \ac{rst} est SHIFT-REDUCE.  
...
\end{itemize}
Un exemple d'une analyse \ac{rst} en utilisant la méthode SHIFT-REDUCE est illustré das la figure \ref{fig:rst-shred-yu-al}.
...
Le composant ``Orancle" doit être entraîné afin de décider l'opération suivante. 
...
On peut aussi utiliser les \keyword[E]{embedding} avec un réseau de neurone comme la méthode proposée dans \cite{2018-yu-al}\footnote{Embedding pour RST : \url{https://github.com/yunan4nlp/NNDisParser}}. 
...
Cette représentation et la concaténation du embedding du mot $w_i$ et de sa catégorie grammaticale $t_i$ comme indiqué dans l'équation \ref{eq:rst-embedding-entree}
\begin{equation}\label{eq:rst-embedding-entree}
x_i^w = embedding(w_i) \oplus embedding(t_i)
\end{equation}
Les mots d'une phrase $w_1 w_2 \ldots w_m$ sont passés par un réseau récurrent Bi-LSTM pour avoir une représentation séquentielle comme indiqué dans l'équation \ref{eq:rst-embedding-seqrep}.
...
Pour ce faire, on utilise un autre réseau récurrent Bi-LSTM comme indiqué dans l'équation \ref{eq:rst-embedding-eduseqrep}
...
Une méthode est d'utiliser \keyword[B]{BERT} avec les parties séparées par \keyword{[SEP]} comme entrée. 
Dans la sortie \keyword{[CLS]}, on attache un réseau de neurones à propagation avant afin d'inférer la connective.
Une autre méthode similaire est l'utilisation de deux réseaux LSTM pour représenté chacune des deux parties. 
...
\begin{figure}[!ht]
	...
	\caption{Une architecture pour la détection de relations PDTB implicites \cite{2020-liang-al}}
...
\begin{table}[!ht]
	...
	\begin{tabular}{p{.2\textwidth}p{.2\textwidth}p{.2\textwidth}}
		...
		\hline\hline
	\end{tabular}
	\caption{Les transitions de la théorie de centralité }
