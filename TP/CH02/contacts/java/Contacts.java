...
public class Contacts {
	...
	private static Map<String, Map<Pattern, String>> genererRegs(){
		...
		Map<Pattern, String> socials_remp = new HashMap<>();
		socials_remp.put(Pattern.compile("https:\\/\\/(\\w+)\\/([^/]+)"), "$1:$2");
	...
	public static Map<String, List<String>> traiterFichier(String url) {
		...
		resultat.put("mails", new ArrayList<String>());
		resultat.put("socials", new ArrayList<String>());
		resultat.put("tels", new ArrayList<String>());
	...
	public static Map<String, Map<String, Map<String, Integer>>> traiterDossier(String url){
		...
		for (File fichier: fichiers){
			...
			contact.put("socials", traiterStats(resultat.get("socials")));
		...
		try {
			...
			while((l = f.readLine()) != null) {
				...
				if (! contacts.containsKey(info[0])) {
					...
					contact.put("socials", new HashMap<String, Integer>());
	...
	public static Comparaison comparer(
		...
		comp.P = (SYS == 0)? 0.0 : ((double)INT) / SYS;
	  comp.F1 = (comp.R+comp.P == 0)? 0.0 : 2 * comp.P * comp.R / (comp.P + comp.R);
	  return comp;
	}
